...
export async function getBrandsByUser(userId) {
  ...
  try {
    // Traer los campos clave para que el selector funcione sin fetch adicional
    const query = `
      SELECT
        id,
        name,
    ...
    return rows.map(brand => ({
      id: brand.id,
      name: brand.name,
      nombre: brand.nombre || brand.name,
      rubro: brand.rubro || '',
      tipografia_principal: brand.tipografia_principal || '',
      color_primario: brand.color_primario || '',
      color_secundario: brand.color_secundario || '',
      personalidad: brand.personalidad || '',
      tono: brand.tono || '',
